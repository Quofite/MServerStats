...
public class TimeStatsCaching extends Thread {
    ...
    public void run() {
        ...
        for (String time : times) {
            try {
                sql = String.format("SELECT SUM(online) FROM statistics WHERE to_char(time, 'HH24:MI')='%s';", time);
                ...
                sql = String.format(Locale.US, "INSERT INTO timestatscache(time, average_online) VALUES ('%s', %f);", time, 0.0);
                template.execute(sql);
            }
        }
    }
